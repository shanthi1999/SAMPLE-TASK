...
router.patch('/edit/:id', [auth, member], async (req, res) => {
  try {
    const { title, description, label,labelText,priority } = req.body;
    if (title === '') {
...
//add comment
router.patch('/addcomment/:id', [auth, member], async (req, res) => {
...
//all comment
router.patch('/allcomments/:id', [auth, member], async (req, res) => {
