...
const Members = () => {
  ...
  const handleInputValue = async (newInputValue) => {
    ...
    if (newInputValue && newInputValue !== "") {
      const search = (
        await axios.get(`http://localhost:5000/api/users/${newInputValue}`)
      ).data.slice(0, 5);
    ...
    <div className="board-members-wrapper">
      ...
      <div className="invite">
        <Autocomplete
          ...
          onChange={(e, newMember) => setUser(newMember)}
          inputValue={inputValue}
          onInputChange={(e, newInputValue) => searchUserOptions(users, newInputValue)}
          options={searchByMembers}
          getOptionLabel={(member) => member.email}
          className="search-member"
          renderInput={(params) => (
            <TextField
